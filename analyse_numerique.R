...
stats_GOT_nb_personnages <-
  ...
  as.data.table()
stats_GOT_nb_personnages <- stats_GOT_nb_personnages[ , lapply(.SD, function(x) ifelse(is.na(x), 0, x))]

stats_GOT_nb_lignes <-
  dcast(stats_GOT, N_serie ~ allegiance, value.var = "nb_lignes_dialogue")%>%
  as.data.table()
stats_GOT_nb_lignes <- stats_GOT_nb_lignes[ , lapply(.SD, function(x) ifelse(is.na(x), 0, x))]

stats_GOT_nb_mots <-
  dcast(stats_GOT, N_serie ~ allegiance, value.var = "nb_mots")%>%
  as.data.table()
stats_GOT_nb_mots <- stats_GOT_nb_mots[ , lapply(.SD, function(x) ifelse(is.na(x), 0, x))]
...
tableaucroise_2D_GOT <- function(data, dimension1, dimension2, statistique){
  result <-
    dcast(data, dimension1 ~ dimension2, value.var = "nb_mots")%>%
    as.data.table()
  result <- result[ , lapply(.SD, function(x) ifelse(is.na(x), 0, x))]
  return((result))
}
